...
Route::middleware('tenant.resolve')->prefix('admin')->name('tenant.admin.')->group(function (): void {
    ...
    Route::middleware('tenant.auth')->group(function (): void {
        ...
        Route::middleware(['tenant.subscription', 'tenant.admin.access'])->group(function (): void {
            ...
            Route::post('/plan/change', [\App\Http\Controllers\Tenant\TenantRecurringSubscriptionController::class, 'change'])->middleware('throttle:6,1')->name('plan.change');
            Route::middleware('tenant.entitlement:AI_CORE')->group(function (): void {
                ...
                Route::get('/ai-handoffs', [AiHumanHandoffController::class, 'index'])->name('ai-handoffs.index');
                Route::post('/ai-handoffs/{handoff}/take', [AiHumanHandoffController::class, 'take'])->middleware('throttle:10,1')->name('ai-handoffs.take');
                Route::post('/ai-handoffs/{handoff}/release', [AiHumanHandoffController::class, 'release'])->middleware('throttle:10,1')->name('ai-handoffs.release');
                Route::post('/ai-conversations/{conversation}/human-messages', [AiHumanHandoffController::class, 'send'])->middleware('throttle:10,1')->name('ai-conversations.human-messages.store');
                ...
                Route::get('/ai-knowledge/{knowledgeSource}', [AiKnowledgeController::class, 'show'])->name('ai-knowledge.show');
            ...
            Route::middleware('tenant.workspace.operational')->group(function (): void {
            ...
            Route::get('/configuracion/entregas', [TenantDeliveryProofOptionController::class, 'index'])->middleware('tenant.workspace.operational')->name('delivery-proof-options.index');
            ...
            Route::post('/configuracion/entregas', [TenantDeliveryProofOptionController::class, 'store'])->middleware('tenant.workspace.operational')->name('delivery-proof-options.store');
            Route::put('/configuracion/entregas/{option}', [TenantDeliveryProofOptionController::class, 'update'])->middleware('tenant.workspace.operational')->name('delivery-proof-options.update');
            Route::get('/operations', [TenantOperationController::class, 'index'])->middleware('tenant.workspace.operational')->name('operations.index');
            Route::get('/operacion', [TenantLogisticsController::class,'index'])->middleware('tenant.workspace.operational')->name('logistics.index');
            Route::post('/operacion/servicios', [TenantLogisticsController::class,'service'])->middleware('tenant.workspace.operational')->name('logistics.services.store');
            Route::put('/operacion/servicios/{service}', [TenantLogisticsController::class,'updateService'])->middleware('tenant.workspace.operational')->name('logistics.services.update');
            Route::post('/operacion/servicios/{service}/tarifas', [TenantLogisticsController::class,'pricing'])->middleware('tenant.workspace.operational')->name('logistics.pricing.store');
            Route::post('/operacion/cobertura', [TenantLogisticsController::class,'coverage'])->middleware('tenant.workspace.operational')->name('logistics.coverage.store');
            Route::post('/operacion/cobertura/importar', [TenantLogisticsController::class,'import'])->middleware('tenant.workspace.operational')->name('logistics.coverage.import');
            Route::patch('/operacion/cobertura/{coverage}', [TenantLogisticsController::class,'toggleCoverage'])->middleware('tenant.workspace.operational')->name('logistics.coverage.toggle');
            Route::delete('/operacion/cobertura/{coverage}', [TenantLogisticsController::class,'removeCoverage'])->middleware('tenant.workspace.operational')->name('logistics.coverage.remove');
            Route::post('/operacion/paquetes', [TenantLogisticsController::class,'package'])->middleware('tenant.workspace.operational')->name('logistics.packages.store');
            ...
            Route::get('/delivery-proofs/{proof}/{kind}', [DeliveryEvidenceController::class, 'tenant'])->middleware('tenant.workspace.operational')->name('delivery-proofs.evidence');
            Route::get('/delivery-failures/{attempt}/photo', [DeliveryEvidenceController::class, 'tenantFailed'])->middleware('tenant.workspace.operational')->name('delivery-failures.photo');
