...
<section class="tenant-pending-grid">@foreach(['DOMINIO'=>'Pendiente ZN-02','PRODUCCIÓN'=>'Pendiente','SANDBOX'=>'No contratado','USAGE'=>'Pendiente','BILLING'=>'Pendiente Billing SaaS'] as $heading=>$pending)<article class="card pending-tile"><small>{{ $heading }}</small><strong>{{ $pending }}</strong></article>@endforeach</section>
@endsection
