...
<section class="card members-card"><h2>Usuarios</h2><div class="member-stats"><strong>{{ $tenant->memberships_count }} usuarios</strong><span>{{ $tenant->owners_count }} owner</span><span>{{ $tenant->active_memberships_count }} activos</span></div><a class="btn" href="{{ route('network.tenants.members.index',$tenant) }}">ADMINISTRAR USUARIOS</a></section>
