...
@foreach($groups as $group=>$groupNodes)<section class="launch-group"><h2 class="section-title">{{ str_replace('_',' ',$group) }}</h2><div class="grid">@foreach($groupNodes as $node)<x-network.app-card :node="$node" />@endforeach</div></section>@endforeach
