...
<section class="z-card" style="margin-top:1rem"><h2>Completadas</h2>@forelse($completedDeliveries as $completed)@php($entry=$completedEarnings->get($completed->local_shipment_id))<div class="driver-status-line"><p><code>{{ $completed->shipment->tracking_number }}</code><br><span class="z-muted">{{ $statusLabels[$completed->shipment->status]??'Completada' }} · {{ $completed->delivered_at }}</span></p>@if($entry)<strong>{{ number_format((float)$entry->amount,2) }} {{ $entry->currency }}</strong>@endif</div>@empty<p class="z-muted">Sin entregas completadas.</p>@endforelse</section>
@endsection
