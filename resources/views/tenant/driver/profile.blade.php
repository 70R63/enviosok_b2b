...
<div class="driver-actions"><a class="z-btn z-btn--outline" href="{{ route(($central?'driver.':'tenant.driver.').'support') }}">Ayuda y soporte</a>@if($central)<button class="z-btn z-btn--outline" type="button" onclick="document.getElementById('install-driver')?.click()">Instalar App</button>@endif<form method="POST" action="{{ route(($central?'driver.':'tenant.driver.').'logout') }}">@csrf<button class="z-btn z-btn--danger z-btn--block">Cerrar sesión</button></form></div>
@endsection
