...
@section('content')
@php($addressText=fn($value)=>is_array($value)?implode(', ',array_filter(array_map(fn($part)=>is_scalar($part)?(string)$part:'',$value))):(string)($value?:'—'))
...
<div class="z-card" style="text-align:left"><div class="z-eyebrow">RUTA</div><p><strong>Origen:</strong> {{ $addressText(data_get($shipment->sender_snapshot,'address')) }}</p><p><strong>Destino:</strong> {{ $addressText(data_get($shipment->recipient_snapshot,'address')) }}</p></div>
