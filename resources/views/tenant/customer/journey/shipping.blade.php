...
@foreach(['sender'=>['REMITENTE','origen'],'recipient'=>['DESTINATARIO','destino']] as $key=>$section)<section class="z-card z-form-section"><div class="z-form-section__head"><div class="z-eyebrow">{{ $section[0] }}</div><h2>Datos de {{ $section[1] }}</h2></div><div class="z-form-grid"><label class="z-field">Nombre<input name="{{ $key }}[name]" value="{{ old($key.'.name') }}" required></label><label class="z-field">Teléfono<input type="tel" name="{{ $key }}[phone]" value="{{ old($key.'.phone') }}" required></label></div><label class="z-field">Número interior opcional<input name="{{ $key }}[interior]" value="{{ old($key.'.interior') }}"></label><label class="z-field">Referencias<textarea name="{{ $key }}[references]">{{ old($key.'.references') }}</textarea></label></section>@endforeach
