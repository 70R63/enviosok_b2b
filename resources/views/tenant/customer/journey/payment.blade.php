...
@section('title','Pago')
@section('content')
@include('tenant.customer.journey.steps',['currentStep'=>6])
...
@else
<div class="z-eyebrow">{{ $tenant->branding?->brand_name ?? $tenant->name }}</div><h1>Completa tu pago</h1><p>Servicio y evidencia seleccionados</p><div class="z-metric-card"><span>Total</span><strong>${{ number_format((float)$checkout->total_amount,2) }} {{ $checkout->currency }}</strong></div>
