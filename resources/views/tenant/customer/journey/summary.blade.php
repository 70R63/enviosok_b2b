...
<div><span>Subtotal</span><strong>${{ number_format((float)($checkout->subtotal_amount ?? $checkout->total_amount),2) }}</strong></div>
<div><span>{{ $taxLabel }}</span><strong>${{ number_format((float)($checkout->tax_amount ?? 0),2) }}</strong></div>
