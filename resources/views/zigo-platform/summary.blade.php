...
<div class="summary"><div><span class="muted">Empresa</span><br><strong>{{ $application->company_name }}</strong></div><div><span class="muted">Contacto</span><br><strong>{{ $application->contact_name }} {{ $application->contact_last_name }}</strong><br>{{ $application->contact_email }}</div><div><span class="muted">Plan</span><br><strong>{{ $snapshot['plan']['name'] }}</strong></div><div><span class="muted">Periodicidad</span><br>{{ $application->billing_period==='annual'?'Anual':'Mensual' }}</div><div><span class="muted">Subdominio</span><br><strong>{{ $application->requested_subdomain }}{{ config('zigo_onboarding.tenant_subdomain_suffix', '') }}.{{ config('zigo_onboarding.subdomain_base') }}</strong></div><div><span class="muted">Capacidad incluida</span><br>{{ !empty($snapshot['plan']['included_operations']) ? 'Hasta '.number_format($snapshot['plan']['included_operations']).' envíos incluidos por periodo' : 'Según capacidades del plan' }}</div></div>
...
<div class="summary"><div>Subtotal</div><div>${{ $snapshot['subtotal'] }} {{ $snapshot['currency'] }}</div><div>IVA (16%)</div><div>${{ $snapshot['tax_amount'] }} {{ $snapshot['currency'] }}</div><div class="total">Total</div><div class="total">${{ $snapshot['total'] }} {{ $snapshot['currency'] }}</div></div>
