...
<head>
    ...
    <style>
        ...
        .btn{display:inline-block;background:#4361ee;color:white;text-decoration:none;font-weight:800;padding:10px 14px;border-radius:9px;border:none;cursor:pointer;font-size:14px}
...
<div class="layout">
    ...
    <main class="content">
        <div class="title">
            @if(request('commercial_status') === 'prospecto')
                Prospectos
            @else
                Clientes
            @endif
        ...
        <div class="subtitle">
            Administración comercial de clientes B2C, B2B, API, mixtos y prospectos.
        </div>
                        ...
                        @forelse($clients as $client)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($client->source)
                                        <div class="muted">Origen: {{ $client->source }}</div>
                                    @endif
                                </td>

                                <td>
                                    <span class="badge badge-blue">
                                        {{ $client->client_type_label ?? strtoupper($client->client_type) }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    {{ $client->commercial_status_label ?? ucfirst($client->commercial_status) }}
                                </td>
                                ...
                                <td>
                                    @php
                                        $leadStatus = $client->lead_status ?? 'nuevo';
                                    @endphp

                                    <strong>{{ ucfirst(str_replace('_', ' ', $leadStatus)) }}</strong>

                                    @if(is_null($client->reviewed_at) && $client->commercial_status === 'prospecto')
                                ...
                                <td>
                                    @php
                                        $priority = $client->lead_priority ?? 'media';
                                ...
                                <td>
                                    {{ $client->next_follow_up_at ? $client->next_follow_up_at->format('d/m/Y H:i') : '-' }}
                                </td>

                                <td>
                                    <div class="actions">
                                        <a class="btn" href="{{ route('crm.clientes.edit', $client) }}">
                                            Editar / Seguimiento
                                        </a>

                                        <form method="POST"
                                              action="{{ route('crm.clientes.destroy', $client) }}"
