...
{
 protected function setUp():void{parent::setUp();$this->app['env']='staging';config()->set('zigo_domains.routing_enabled',true);config()->set('zigo_payments.providers.mercado_pago.webhook_secret','stage-secret-placeholder');}
 ...
 public function test_payment_edge_is_host_bound_post_only_and_secure():void{
  ...
  $this->get('https://rapidgo-stage.zigo-envios.com/payments/health')->assertStatus(400);
  $this->get('https://'.$payments.'/api/payments/mercado-pago/webhook')->assertStatus(405);
  $this->postJson('https://rapidgo-stage.zigo-envios.com/api/payments/mercado-pago/webhook')->assertStatus(400);
 ...
 public function test_callback_is_central_and_does_not_require_tenant_cookie():void{
  ...
  $this->get('https://rapidgo-stage.zigo-envios.com/payments/mercado-pago/oauth/callback?state=invalid&code=x')->assertStatus(400);
 }
 public function test_driver_and_network_surfaces_do_not_leak_across_hosts():void{
  ...
  $this->get('https://network-stage.zigo-envios.com/driver/manifest.webmanifest')->assertStatus(400);
  $this->get('https://'.config('zigo_surfaces.network.host').'/network/login')->assertOk();
  $this->get('https://rapidgo-stage.zigo-envios.com/network/login')->assertStatus(400);
 }
