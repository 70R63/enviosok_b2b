...
final class NetworkTenantB2cTest extends TestCase
{
    ...
    public function test_verified_host_is_branded_and_unknown_host_is_404(): void
    {
        ...
        $this->get($this->url($tenant, '/cotizar'))->assertOk()->assertSee('Rapid Alpha')->assertSee('Código postal de origen')
            ->assertSee('data-options="origin"', false)
    ...
    public function test_tenant_storefront_routes_are_public_branded_and_isolated_from_corporate_hosts(): void
    {
        ...
        $home = $this->get($this->url($tenant, '/'))->assertOk()
            ...
            ->assertSee('data-postal-display="origin"', false)
            ->assertSee('data-postal-display="destination"', false)
            ->assertDontSee('<select name="origin_settlement"', false)
            ->assertDontSee('<select name="destination_settlement"', false)
            ->assertSee('`${postal} - ${name}`', false)
