...
final class XpertaIntegrationController extends Controller
{
    ...
    public function token(XpertaStageIntegrationTester $tester): RedirectResponse { $this->executable(); return back()->with('probe_result',$tester->token()); }
    public function frequency(Request $request, XpertaStageIntegrationTester $tester): RedirectResponse
    {
        $this->executable(); $data=$request->validate(['origin'=>['required','regex:/^\d{5}$/'],'destination'=>['required','regex:/^\d{5}$/']]);
    ...
    {
        $this->executable(); $data=$request->validate(['origin'=>['required','regex:/^\d{5}$/'],'destination'=>['required','regex:/^\d{5}$/'],'weight'=>['required','numeric','gt:0','max:1000'],'length'=>['required','numeric','gt:0','max:500'],'width'=>['required','numeric','gt:0','max:500'],'height'=>['required','numeric','gt:0','max:500'],'service'=>['required',Rule::in(['terrestre','diasig'])],'declared_value'=>['required','numeric','min:0','max:1000000']]);
