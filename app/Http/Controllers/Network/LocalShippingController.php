...
use App\Domain\Shipping\Local\Models\{LocalShipment,LocalShippingService,LocalShippingZone};
...
final class LocalShippingController extends Controller
{
    public function index()
    {
        return view('network.local-shipping.index', [
            ...
            'shipments' => LocalShipment::with('tenant')->latest()->paginate(25),
        ]);
    }
    ...
    {
        $data=$request->validate(['postal_code'=>['required','regex:/^\d{5}$/']]); $coverage->assign($zone,$data['postal_code']);
    ...
    public function toggleZone(LocalShippingZone $zone){$zone->update(['status'=>$zone->status==='active'?'inactive':'active']);return back();}
    public function toggleService(LocalShippingService $service){$service->update(['status'=>$service->status==='active'?'inactive':'active']);return back();}
}
