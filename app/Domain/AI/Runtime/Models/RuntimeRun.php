...
final class RuntimeRun extends AiTenantModel
{
    ...
    {
        return ['uuid','agent_id','agent_version_id','purpose','execution_mode','status','provider_code','model_code','provider_http_status','provider_error_type','provider_error_code','provider_error_param','provider_request_id','client_request_id','provider_response_id','input_units','cached_input_units','output_units','total_units','estimated_cost_microusd','input_price_microusd_per_million','cached_input_price_microusd_per_million','output_price_microusd_per_million','citation_count','confidence','needs_handoff','handoff_reason','latency_ms','safe_error_code','created_by_user_id','started_at','completed_at','failed_at'];
    }
    ...
    public function complete(AuthorizedAiLifecycleActor $actor, ModelResponseData $model, AgentRuntimeResponseData $response, array $pricing): void
    {
        $this->ensureStarted(); $this->assertLifecycleActor($actor); $cost=self::cost($model,$pricing);
    ...
    public function fail(AuthorizedAiLifecycleActor $actor, string $code, ?ProviderFailureData $failure=null): void
    {
        $this->ensureStarted();$this->assertLifecycleActor($actor);
    ...
    public function skip(AuthorizedAiLifecycleActor $actor): void
    {
        $this->ensureStarted();$this->assertLifecycleActor($actor);$this->persistNamedLifecycle($this->mutable(),function():void{$this->status=RuntimeRunStatus::SkippedNoKnowledge;$this->confidence='low';$this->needs_handoff=true;$this->handoff_reason='insufficient_knowledge';$this->completed_at=now();});
    }
    ...
    private function mutable(): array { return array_values(array_diff($this->immutableIdentityAttributes(),['uuid','agent_id','agent_version_id','purpose','execution_mode','provider_code','model_code','created_by_user_id','started_at'])); }
