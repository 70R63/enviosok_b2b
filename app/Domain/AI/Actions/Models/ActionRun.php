...
final class ActionRun extends AiTenantModel
{
    ...
    protected $casts=['effect'=>ActionEffect::class,'status'=>ActionRunStatus::class,'input'=>'encrypted:array','output'=>'encrypted:array','requested_at'=>'datetime','confirmed_at'=>'datetime','started_at'=>'datetime','completed_at'=>'datetime','failed_at'=>'datetime'];
    public function getRouteKeyName():string{return'uuid';}
    public function isAiAppendOnly():bool{return true;}
    protected function immutableIdentityAttributes():array{return['uuid','agent_id','agent_version_id','contract_version_id','conversation_id','source_message_id','runtime_run_id','action_key','effect','idempotency_key','input','status','output','safe_error_code','requested_at','confirmed_at','confirmed_by_user_id','started_at','completed_at','failed_at'];}
    ...
    public function succeed(array$output):void{if($this->originalAiStatus()!==ActionRunStatus::Executing->value)throw new \DomainException('Action is not executing.');$this->persistNamedLifecycle(['status','output','completed_at'],function()use($output){$this->status=ActionRunStatus::Succeeded;$this->output=$output;$this->completed_at=now();});}
    public function fail(string$code):void{if($this->originalAiStatus()!==ActionRunStatus::Executing->value||!in_array($code,['action_not_registered','action_not_allowed','action_output_invalid','action_failed'],true))throw new \DomainException('Action cannot fail with that state or code.');$this->persistNamedLifecycle(['status','safe_error_code','failed_at'],function()use($code){$this->status=ActionRunStatus::Failed;$this->safe_error_code=$code;$this->failed_at=now();});}
