...
final class TenantB2cQuoteService
{
    ...
    public function quote(Tenant $tenant, array $data): array
    {
        ...
        foreach($eligible as $service){
            ...
            $options[]=['snapshot_uuid'=>$snapshot->uuid,'service'=>$service->name,'service_code'=>$service->code,'sla'=>$service->sla_text,'price'=>$price->totalAmount,'currency'=>$price->currency,'distance_meters'=>$snapshot->distance_meters,'preliminary'=>$preliminary];
        }
