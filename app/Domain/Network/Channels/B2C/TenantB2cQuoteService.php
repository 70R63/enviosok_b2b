...
final class TenantB2cQuoteService
{
    ...
    public function quote(Tenant $tenant, array $data): array
    {
        ...
        foreach($eligible as $service){
            ...
            $matched['_commercial'] = ['subtotal'=>(string)$price->totalAmount,'tax'=>'0.00','total'=>(string)$price->totalAmount,'base'=>(string)$price->baseAmount,'distance_overage'=>(string)$price->overageAmount];
            ...
            $options[]=['snapshot_uuid'=>$snapshot->uuid,'service'=>$service->name,'service_code'=>$service->code,'provider'=>'ZIGO Local','sla'=>$service->sla_text,'subtotal'=>$price->totalAmount,'tax'=>'0.00','price'=>$price->totalAmount,'currency'=>$price->currency,'origin'=>$origin,'destination'=>$destination,'package'=>['type'=>$data['tipo_envio'],'weight'=>(string)$data['peso']],'distance_meters'=>$snapshot->distance_meters,'preliminary'=>$preliminary];
        }
