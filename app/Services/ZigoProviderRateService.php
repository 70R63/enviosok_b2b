...
class ZigoProviderRateService
{
    ...
    public function getOptionsForCotizacion(
        ...
        if ($provider === 'xperta') {
            if (!config('services.xperta.enabled', false)) {
                throw new RuntimeException(
                    'Xperta está seleccionado, pero XPERTA_ENABLED no está activo.'
                );
            }

            try {
                return $this->xpertaQuoteService->options(
                    $cotizacion
                );
            } catch (Throwable $exception) {
                Log::error('ZIGO Provider Rate - Error Xperta', [
                    'cotizacion_id' => $cotizacion->id,
                    'message' => $exception->getMessage(),
                ]);

                if (!config(
                    'services.xperta.fallback_to_legacy',
                    false
                )) {
                    throw $exception;
                }
                ...
                Log::warning(
                    'ZIGO Provider Rate - Fallback legacy habilitado',
                    ['cotizacion_id' => $cotizacion->id]
                );
            }
        }
        ...
        return [
            $this->getLegacyEstafetaOption($cotizacion),
        ];
    }
    ...
    private function getLegacyEstafetaOption(
        ...
        $basePrice = $this->getLegacyEstafetaBasePrice(
            $cotizacion
        );
    ...
    private function getLegacyEstafetaBasePrice(
        ...
        try {
            $data = $this->buildLegacyPayload($cotizacion);

            $cotizador = new EstafetaCreacion();
            $cotizador->soloCotizacion($data);

            $response = $cotizador->getResponse();
            $basePrice = $this->extractLegacyTotal($response);

            if ($basePrice <= 0) {
                throw new RuntimeException(
                    'La cotización Estafeta no regresó un total válido.'
                );
            }
            ...
            Log::error(
                'ZIGO Provider Rate - Error cotizando Estafeta legacy',
                [
                    'cotizacion_id' => $cotizacion->id,
                    'message' => $exception->getMessage(),
                ]
    ...
    private function buildLegacyPayload(
        ...
        [$largo, $ancho, $alto] = $this->parseDimensions(
            $cotizacion->medidas
        );
        ...
        return [
            ...
            'bSeguro' => 'false',
            'tipoPagoId' => 1,
            'contenido' => $cotizacion->contenido
                ?? 'Mercancía general',
        ];
    }
    ...
    private function extractLegacyTotal($response): float
    {
        ...
        throw new RuntimeException(
            'No se encontró total en la respuesta Estafeta legacy.'
        );
    }
}
